...
interface BookstoreAPI {
    ...
    fun updateBook(@PathVariable isbn: String, @Valid @RequestBody bookDTO: BookDTO) : ResponseEntity<*>

    //5
    @Operation(summary = "Partially update a book", description = "Updates only the provided fields of a book.")
    @ApiResponses(value = [
        ApiResponse(responseCode = "200", description = "Book updated"),
        ApiResponse(responseCode = "400", description = "Invalid field values", content = [Content(
            schema = Schema(implementation = ErrorResponse::class))]),
        ApiResponse(responseCode = "404", description = "Book not found", content = [Content(
            schema = Schema(implementation = ErrorResponse::class))])
    ])
    ...
    @GetMapping("/books/{isbn}/reviews/{reviewId}")
    fun getSingleReview(@PathVariable isbn: String, @PathVariable reviewId: String) : ResponseEntity<ReviewDTO>
    ...
    @ApiResponses(value = [
        ...
        ApiResponse(responseCode = "400", description = "Validation error (e.g., rating not 1-5)", content = [Content(
            schema = Schema(implementation = ErrorResponse::class))]),
        ApiResponse(responseCode = "404", description = "Book not found", content = [Content(
            schema = Schema(implementation = ErrorResponse::class))])
    ])
    ...
    @ApiResponses(value = [
        ...
        ApiResponse(responseCode = "400", description = "Validation error", content = [Content(
            schema = Schema(implementation = ErrorResponse::class))]),
        ApiResponse(responseCode = "404", description = "Review or Book not found", content = [Content(
            schema = Schema(implementation = ErrorResponse::class))])
    ])
